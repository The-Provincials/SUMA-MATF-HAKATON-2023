    ...
    for i in terms:
        for j in findminterms(i):
            try:
                ...
                pass
def logical_expr_to_binary(expr):
    # Remove whitespace and special characters from expression
    #expr = "".join(c for c in expr if c.isalpha() or c in "*+~")

    # Map each variable to a binary digit
    var_map = {var: i for i, var in enumerate(sorted(set(c for c in expr if c.isupper())))}
    n_vars = len(var_map)

    # Generate all possible input combinations and evaluate the expression for each

    input_combs = [f"{i:0{n_vars}b}" for i in range(2 ** n_vars)]
    expr1 = expr.replace("*", " and ")
    expr1 = expr1.replace("+", " or ")
    expr1 = expr1.replace("~", " not ")

    allExpr = []
    for combination in input_combs:
        tmpExpr = expr1
        for i in range(len(combination)):
            if combination[i] == '1':
                tmpExpr = tmpExpr.replace(chr(ord('A')+i), 'True')
            else:
                tmpExpr = tmpExpr.replace(chr(ord('A')+i), 'False')
        allExpr.append(tmpExpr)
    outputs = [int(eval(tmpExpr)) for tmpExpr in allExpr]
    # Convert the outputs to binary string and then to int
    binary_str = "".join(str(output) for output in outputs)
    #binary_num = int(binary_str, 2)

    return binary_str
def find_minterms(expr):
    n = len(expr)
    return [i for i in range(n) if expr[i] == '1']

with open("izrazi.txt", "r", encoding="utf-8") as file:
    for line in file:
        exp = logical_expr_to_binary(line)

        mt = find_minterms(exp)#[int(i) for i in input("Enter the minterms: ").strip().split()]
        dc = []
        mt.sort()
        minterms = mt+dc
        minterms.sort()
